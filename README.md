...
4. Проверяется корректность структуры
5. Выполнение: 
...
- Экранирование: спецсимволы через \
...
Результат
Создана полноценная командная оболочка с поддержкой всех основных функций современных shell( управление задачами, перенаправления, конвейеры и встроенные команды. принципы работы операционных систем, управления процессами и синтаксического анализа)

замечания: 1)Частичная остановка процессов внутри конвейера не реализована,job control работает на уровне process group

shell.c
1)Добавлены новые заголовки: <termios.h> для работы с raw mode
2)Новые константы: ESC, BACKSPACE, DELETE, MAX_LINE_LENGT
3)Raw mode функции: enable_raw_mode() и disable_raw_mode() для посимвольного ввода
4)Функции редактирования:
redraw_line() - перерисовка строки с курсором
insert_char() - вставка символа
backspace() - удаление символа слева
delete_char() - удаление символа под курсором
move_left()/move_right() - навигация
set_line_from_history() - загрузка из истории
5)Новый read_input() - полностью переписан для поддержки:
Стрелок влево/вправо для навигации
Стрелок вверх/вниз для истории
Backspace и Delete
Enter для завершения ввода
6)Дополнения в shell_t (в shell.h):
Поля для истории команд
Буфер для редактирования строки
7)История команд:
history_add() - добавление команды
history_print() - просмотр истории
